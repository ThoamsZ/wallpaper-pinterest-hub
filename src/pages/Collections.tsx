  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        ...
        `).in('id', currentUser.favor_collections).order('created_at', {
        ascending: false
      });
      if (error) {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto pt-20">
        <div className="mb-8">
          ...
          {selectedCollection && <Button variant="outline" className="mt-4" onClick={() => setSelectedCollection(null)}>
              ← Back to Collections
            </Button>}
        </div>

        {!selectedCollection ? isCollectionsLoading ? <div className="flex items-center justify-center min-h-[60vh]">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div> : collections.length === 0 ? <div className="text-center py-12">
              ...
              {collections.map(collection => <div key={collection.id} className="p-6 rounded-lg border bg-card text-card-foreground shadow-sm hover:shadow-md transition-shadow cursor-pointer relative" onClick={() => navigate(`/collection/${collection.id}`)}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    {getCollectionPreviewImages(collection).map((url, index) => <img key={index} src={url} alt={`Preview ${index + 1}`} className="w-full h-24 object-cover rounded-md" />)}
                  </div>
            ...
            {selectedCollectionWallpapers.length === 0 ? <div className="text-center py-12">
                <p className="text-muted-foreground">No wallpapers in this collection</p>
              </div> : <WallpaperGrid wallpapers={selectedCollectionWallpapers} />}
          </div>}
      </main>
    </div>;
};
